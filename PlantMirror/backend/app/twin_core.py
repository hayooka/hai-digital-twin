...
def load_bundle(
    ...
    plant = GRUPlant(
        n_plant_in=d["n_plant_in"],
        n_pv=d["n_pv"],
        hidden=d["hidden"],
        layers=d["layers"],
        n_scenarios=d["n_scenarios"],
    )
    plant.load_state_dict(d["model_state"])
...
def default_paths() -> Dict[str, Path]:
    ...
    root = here.parent  # hai-digital-twin/
    return {
        "ckpt_dir": root / "outputs" / "gru_scenario_weighted" / "gru_scenario_weighted",
        "split_dir": root / "outputs" / "scaled_split",
        "causal_json": root / "outputs" / "causal_graph" / "parents_full.json",
        "test_csvs": [
            Path(root.parent) / "processed" / "test1.csv",
            Path(root.parent) / "processed" / "test2.csv",
        ],
    }
