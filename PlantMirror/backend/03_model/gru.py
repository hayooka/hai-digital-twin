...
class CCClassifierRegressor(nn.Module):
    """
    CC loop (cooling control) model — GRU-based for full sequence history.

    GRU trunk     : [P1_TIT03, P1_PP04SP] over full 300-step history → hidden state
    Classifier    : Linear(hidden→1)  logit for pump on/off
    Regressor     : Linear(hidden→1)  pump speed (meaningful only when pump is on)

    Upgraded from single-timestep MLP to GRU — uses full sequence context.
    """

    def __init__(self, n_inputs: int = 2, hidden: int = 64, dropout: float = 0.1):
        super().__init__()
        drop = dropout if 2 > 1 else 0.0
        self.gru        = nn.GRU(n_inputs, hidden, num_layers=2,
                                 batch_first=True, dropout=drop)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """
        x: (B, T, n_inputs) or (B, n_inputs)
        → pump_logit (B, 1),  pump_speed (B, 1)
        """
        if x.dim() == 2:
            x = x.unsqueeze(1)
        _, h = self.gru(x)
        h = self.dropout(h[-1])               # last layer hidden state (B, hidden)
