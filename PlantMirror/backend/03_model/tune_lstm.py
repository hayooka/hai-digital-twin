"""
tune_lstm.py — Evolutionary hyperparameter search for LSTMPlant using Optuna CMA-ES.

Same design as tune_gru.py — see that file for full explanation of why CMA-ES
and why only plant hyperparameters are tuned.

Run:
    python 03_model/tune_lstm.py
"""
...
OUT_DIR = Path("outputs/lstm_tune")
...
def objective(trial: optuna.Trial) -> float:
    ...
    cc = LSTMCCClassifierRegressor(n_inputs=2, hidden=CC_HIDDEN, dropout=dropout).to(device)
...
print("Now plug these values into train_lstm.py and run a full training.")
