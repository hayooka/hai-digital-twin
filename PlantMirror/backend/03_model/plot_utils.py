...
def plot_multi_horizon(
    ...
    - Each image shows all horizons (10 min, 30 min, 1h, 1.5h) stacked vertically
    ...
    for sc_id in [0, 1, 2, 3]:
        ...
        # Build consecutive chain with enough windows for the longest horizon
        chain = _get_consecutive_chain(best_start, scenario_labels, sc_id, max_windows_needed)
        
        # Warn if not enough consecutive windows are available
        if len(chain) < max_windows_needed:
            logger.warning(
                f"Scenario {sc_id} ({sc_short}): Only {len(chain)} consecutive windows available. "
                f"Requested {max_windows_needed} windows for {max_horizon_s//60} min horizon. "
                "Plots will be truncated."
            )
        else:
        ...
        for horizon_s in custom_horizons:
            ...
            n_steps = min(n_steps, len(chain))  # Use only available windows
        ...
        for pv_idx in range(n_pv):
            ...
            fig.suptitle(
                ...
                f"Prediction performance across all horizons (10min, 30min, 1h, 1.5h)",
            ...
            for h_idx, hd in enumerate(horizon_data):
                ...
                # Format horizon label nicely
                if hd['horizon_s'] == 600:
                    horizon_label = "10 min"
                elif hd['horizon_s'] == 1800:
                    horizon_label = "30 min"
                elif hd['horizon_s'] == 3600:
                    horizon_label = "1 hour"
                elif hd['horizon_s'] == 5400:
                    horizon_label = "1.5 hours"
                else:
                    horizon_label = f"{hd['horizon_s']}s"
                
                # Subplot title with horizon info
                ax.set_title(
                    f"Horizon: {horizon_label} ({hd['horizon_s']}s) | "
