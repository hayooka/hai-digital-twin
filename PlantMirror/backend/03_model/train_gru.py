...
BEST_DIR = Path("outputs/gru_plant_best")
...
scenario_train = plant_data['scenario_train']      # (N,)  0=normal, 1-3=attack
...
for epoch in range(1, EPOCHS + 1):
    ...
    if pval < best_plant_val:
        ...
        torch.save(ckpt, OUT_DIR  / "gru_plant.pt")
        torch.save(ckpt, BEST_DIR / "gru_plant.pt")  # backup never overwritten by next run
    else:
        patience_counter += 1
...
# ── NRMSE on test set (normal windows only) ───────────────────────────────────
