"""
tune_gru.py — Evolutionary hyperparameter search for GRUPlant using Optuna CMA-ES.

WHY THIS EXISTS:
    Manual hyperparameter selection (trial-and-error) is not reproducible and
    unlikely to find the true optimum. CMA-ES (Covariance Matrix Adaptation
    Evolution Strategy) is an evolutionary algorithm that:
      - Starts with a population of parameter configurations
      - Learns which regions of the search space give better results
      - Adapts its search distribution toward good configurations
      - Prunes bad trials early (MedianPruner) to save time

HOW IT WORKS:
    1. Data is loaded ONCE and shared across all trials (expensive operation).
    2. Each trial samples a new set of hyperparameters and trains GRUPlant for
       N_TRIAL_EPOCHS=40 epochs (quick signal, not full training).
    3. Optuna's MedianPruner kills trials that are worse than the median at
       epoch 10 — saves ~50% of compute.
    4. After N_TRIALS=30 trials, best params are saved to outputs/gru_tune/best_params.json.
    5. Plug best_params.json into train_gru.py and do one full training run.

    WHY ONLY PLANT HYPERPARAMS:
    Controllers converge quickly and their architecture has little effect on the
    final NRMSE. Tuning only the plant (the hard part) keeps trial time short.

Run:
    python 03_model/tune_gru.py
"""
...
SEED           = 42
N_TRIALS       = 30      # number of CMA-ES trials
N_TRIAL_EPOCHS = 40      # epochs per trial (quick signal)
CTRL_HIDDEN    = 64
CTRL_LAYERS    = 2
CC_HIDDEN      = 32
CTRL_LR        = 1e-3
...
def objective(trial: optuna.Trial) -> float:
    ...
    cc = CCClassifierRegressor(n_inputs=2, hidden=CC_HIDDEN, dropout=dropout).to(device)
    ...
    best_val = float("inf")
    # Cap batch at 128 to avoid OOM on 10GB GPU with hidden=512
    batch = min(batch, 128)
    print(f"\n  Trial {trial.number+1}/{N_TRIALS}: "
          f"hidden={hidden} layers={layers} dropout={dropout:.2f} "
          f"lr={lr:.1e} batch={batch} ss_end={ss_end} ss_max={ss_max:.2f}")
    ...
    for epoch in range(1, N_TRIAL_EPOCHS + 1):
        ...
        if epoch % 5 == 0 or epoch == 1:
            print(f"    epoch {epoch:3d}/{N_TRIAL_EPOCHS}  val={pval:.6f}  best={best_val:.6f}")

        # Report to Optuna for pruning — kills bad trials early
        trial.report(pval, epoch)
        if trial.should_prune():
            print(f"    Pruned at epoch {epoch}  (val={pval:.6f})")
...
# ── Run study ─────────────────────────────────────────────────────────────────
# CMA-ES: evolutionary strategy that adapts search distribution toward good configs.
# MedianPruner: after 5 startup trials, prunes any trial worse than median at epoch 10.
...
study   = optuna.create_study(
    ...
    sampler=sampler,
    pruner=pruner,
    study_name="gru_plant_tune",
)

print(f"\nStarting CMA-ES search: {N_TRIALS} trials × {N_TRIAL_EPOCHS} epochs each")
print("Pruner will kill trials worse than median after epoch 10.\n")
...
print("Now plug these values into train_gru.py and run a full training.")
