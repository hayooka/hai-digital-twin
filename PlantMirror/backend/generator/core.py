...
Loads the gru_scenario_weighted checkpoints (GRUPlant + 4 GRUControllers + 1
...
# Anomaly threshold from results.json (gru_scenario_weighted)
...
class GRUPlant(nn.Module):
    ...
    def __init__(
        ...
        # Auxiliary HAIEND head — declared so strict state_dict loading succeeds on
        # checkpoints trained with the HAIEND second head. Not invoked at inference
        # since the generator only consumes PV predictions.
        if n_haiend > 0:
    ...
    def predict(
        ...
        for t in range(T):
            ...
            pv = self.fc(out.squeeze(1))
            outs.append(pv)
        return torch.stack(outs, dim=1)

    # ── Live twin primitives ──────────────────────────────────────────────
    # These decompose the predict() loop into a persistent-state form so a
    # TwinRuntime can maintain (h_plant, pv_twin) across simulated seconds.
    ...
    def step_once(
        self,
        x_cv_t: torch.Tensor,  # (B, n_plant_in) — CV sample for this second
        h: torch.Tensor,        # (L, B, hidden) — decoder hidden state
        pv_prev: torch.Tensor,  # (B, n_pv)
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Advance the decoder one second. Returns (pv_new, h_new)."""
        self.eval()
        dec_in = torch.cat([x_cv_t, pv_prev], dim=-1).unsqueeze(1)
        out, h_new = self.decoder(dec_in, h)
        pv_new = self.fc(out.squeeze(1))
        return pv_new, h_new
...
def load_bundle(
    ...
    state = d["model_state"]
    # Newer haiend checkpoints drop these metadata keys — infer from state_dict.
    emb_dim = state["scenario_emb.weight"].shape[1]
    n_scenarios = d.get("n_scenarios", state["scenario_emb.weight"].shape[0])
    n_plant_in = d.get("n_plant_in", state["encoder.weight_ih_l0"].shape[1] - emb_dim)
    n_pv = d.get("n_pv", state["fc.3.weight"].shape[0])
    plant = GRUPlant(
...
def default_paths() -> Dict[str, Path]:
    """Resolve local generator paths (standalone — artifacts live alongside this file)."""
    here = Path(__file__).resolve().parent
    return {
        "ckpt_dir": here / "weights",
        "split_dir": here / "scalers",
        "causal_json": here / "weights" / "parents_full.json",
        "test_csvs": [here / "replay" / "test1.csv"],
    }
