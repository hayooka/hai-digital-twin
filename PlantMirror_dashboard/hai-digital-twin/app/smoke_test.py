...
from twin_runtime import classify_window, _attack_clf
...
def main() -> None:
    ...
    # 7. 4-class attack classifier --------------------------------------------
    if _attack_clf is None:
        print("  [skip] attack classifier PKL not found — skipping")
    else:
        rng = np.random.default_rng(42)
        dummy_window = rng.standard_normal((180, 5)).astype(np.float32)
        label = classify_window(dummy_window)
        assert label in {"Normal", "AP_no", "AP_with", "AE_no"}, f"unexpected label: {label}"
        _ok(f"attack classifier loaded and predicts valid labels (dummy → '{label}')")

    # 8. Causal rank_suspects per PV -----------------------------------------
    for pv in PV_COLS:
